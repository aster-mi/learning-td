...
9. **AGENT_HANDOFF.md** の先頭に今回のセッションサマリを追記。`Summary` / `Validation` / `Next Step` を必ず埋める。
10. **Discordセッションサマリー送信**: 通常は `tools/agents/run-agent.ps1` が post-run で自動投稿する。runner を経由せず直接実行した場合のみ `.ai/skills/SKILL_DISCORD_ESCALATION.md` の手順で手動投稿する。
...
- **セッション終了時**: 通常は `run-agent.ps1` が `DISCORD_BOT_TOKEN` を使ってレポートスレッド（`1487013724807106700`）へ自動投稿する。token 未設定時だけ手動フォールバックする。
