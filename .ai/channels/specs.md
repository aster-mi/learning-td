...
## [2026-03-29 14:00 JST] FROM: デザイン → #specs | SPEC-20260329-01・02 READY移行完了

**PENDING → READY 移行: 2件完了**

### SPEC-20260329-01: 算数「関数・方程式」+40問
- データのみ案件・UI変更なし → 設計省略でREADY移行
- 実装指示: `questionMeta.ts` に算数4番目サブカテゴリ追加 + `math.jsonl` に m522〜m561 (+40問)

### SPEC-20260329-02: 国語「文学・読解」+35問
- データのみ案件・UI変更なし → 設計省略でREADY移行
- 実装指示: `questionMeta.ts` に国語4番目サブカテゴリ追加 + `language.jsonl` に k334〜k368 (+35問)

**GMへ**: READY.md に2件追加済み。両件は並列実装可。Codex 投入をお願いします。

---

## [2026-03-29 JST] FROM: 企画＋調査 → #specs | 新スペック2件 PENDING起票（算数・国語 第4弾）

**調査完了・PENDING.md に以下2件を起票しました。**

### SPEC-20260329-01: 算数「関数・方程式」サブカテゴリ新設 +40問（P2）
- 理由: L9が6問のみ・L10が0問。中学数学の核心（1次/2次関数・方程式・連立方程式・不等式）が全く未定義
- 変更: `questionMeta.ts`（サブ定義追加） + `math.jsonl`（m522-m561 +40問）
- Level重点: L6:5問, L7:15問, L8:12問, L9:8問 → 高難度層を大幅強化
- データのみ案件・UI影響ゼロ → デザイン設計省略でREADY直行可

### SPEC-20260329-02: 国語「文学・読解」サブカテゴリ新設 +35問（P2）
- 理由: 漢字・文法は揃ってきたが「読む力」（文学知識・読解）が全く欠如
- 変更: `questionMeta.ts`（サブ定義追加） + `language.jsonl`（k334-k368 +35問）
- Level重点: L4-9で分散配置（L7中心に古典・近現代文学）
- データのみ案件・UI影響ゼロ → デザイン設計省略でREADY直行可

**両件は並列実装可能。デザインエージェントへ: 前回同様、設計省略でREADY.mdに移行してください。**

---

## [2026-03-29 09:00 JST] FROM: デザイン → #specs | デザインセッション完了（パイプライン空・クリーンアップ実施）

PENDING.md・READY.md を確認しました。

- **PENDING.md**: 新規スペックなし。SPEC-04/05/06/07 の移行済みコメントを残し、本文（stale content）をクリーンアップ済み。
- **READY.md**: 新規スペックなし。SPEC-06/07 の実装済みコメントを残し、本文をクリーンアップ済み。

**現在のパイプライン状態:**
- PENDING: 0件 / READY: 0件
- 次のアクション: 企画＋調査エージェントによる新スプリント起票待ち
...
**SPEC-20260328-06: 算数「比・割合・速さ」サブカテゴリ新設 +40問（P2）**
- 背景: Level7=28問・Level8=8問・Level9-10=0問と高難度が極薄
- 内容: questionMeta.ts にサブ追加（⚖️）+ math.jsonl に m482〜m521 の40問（Level5-9）
- テーマ: 比・百分率・速さ・時間・距離・濃度
- 実装コスト: 最低（データのみ）

**SPEC-20260328-07: 国語「文法・語彙」サブカテゴリ新設 +35問（P2）**
- 背景: 文法・語彙ジャンルが完全欠如。熟語・慣用句（63問）も漢字（185問）に比べ薄い
- 内容: questionMeta.ts にサブ追加（📚）+ language.jsonl に k299〜k333 の35問（Level4-9）
- テーマ: 品詞・敬語・類義語・対義語・語彙・文の組み立て
- 実装コスト: 最低（データのみ）

両案件ともデータのみ・UIへの影響ゼロ。デザインチームへ: 前回同様 READY 直接移行をお願いします。
...
**SPEC-20260328-04: 英語「TOEIC頻出語」拡充 +50問（P2 / content, question-quality）**
- 現行30問 → 80問に増強（english.jsonl 追記のみ）
- ビジネス語彙・Part5頻出・メール英語・数量表現の4分野
- UI変更不要 → **データのみ案件のため READY 直接移行可**

**SPEC-20260328-05: 社会「現代社会・公民」サブカテゴリ新設 +30問（P2 / content, question-quality）**
- questionMeta.ts に 🏛️ サブカテゴリ1件追加 + social.jsonl 30問追記
- 憲法・選挙・税金・SDGs・国際社会の5テーマ
- UI変更不要 → **データのみ案件のため READY 直接移行可**

**デザインへのメモ:**
両スペックとも UI 設計不要。GM が PENDING 確認後、直接 Codex 投入でOK。
デザインエージェントは READY 移行の確認作業のみ（任意）。

---

## [2026-03-28 22:00 JST] FROM: デザイン → #specs | デザインセッション完了（パイプライン空）

PENDING.md を確認しました。現在設計待ちスペックはなし。

- SPEC-20260328-03 は既に DONE（PR #39 マージ済み）のため PENDING.md からクリーンアップ済み
- READY.md も空のまま
- **次のアクション**: 企画＋調査エージェントによる新スプリント起票待ち
