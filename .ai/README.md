...
| エージェント | タスクID | 起動タイミング | 役割 |
|---|---|---|---|
| CEO | `learning-td-ceo` | 毎日 7:00 | 戦略方針の決定 |
| 企画＋調査 | `learning-td-planning` | 毎日 8:30 | トレンド調査・機能仕様化 |
| デザイン | `learning-td-design` | 毎日 9:30 | UI/UX設計・仕様のREADY化 |
| GM＋レビュー | `learning-td-enhance` | 5時間ごと | Codex投入・PRレビュー・マージ |
| Codex（実装） | GMから起動 | GMセッション内 | コード実装（最大10並列） |
...
次のGMセッション（最大5時間以内）で自動的に拾って処理します。
...
Claude Code のサイドバー「**Scheduled**」セクションから確認・管理できます：

| 操作 | 方法 |
|---|---|
| 今すぐ実行 | 該当タスクの「Run now」 |
| 一時停止 | タスクの enable/disable トグル |
| 内容確認 | タスクをクリックして SKILL.md を表示 |
...
GMセッションが5時間ごとに実行されるので最大5時間待てば処理されます。
すぐに処理したい場合はサイドバーから「Run now」で `learning-td-enhance` を即時実行してください。
