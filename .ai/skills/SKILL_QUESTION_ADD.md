...
- **正解は1つだけにする**（複数候補が成立する問題文を作らない）
- 特になぞなぞでは、選択肢の中に「条件を満たす別解」がないかを必ず確認する
  - 例: 「足があるのに歩けないものは？」に `テーブル` と `イス` を同時に置かない

## なぞなぞ追加時の追加チェック（必須）
1. 問題文だけ読んで、正解候補が2つ以上思いつかないかを確認
2. 各選択肢について「問題文の条件を満たすか」を1つずつ確認
3. 正答以外に成立する選択肢がある場合は、問題文を絞る
   - 例: 「上にお皿をのせられるもの」「書いたり置いたりする家具」など条件を追加

## 雑学追加時の追加チェック（必須）
1. 時点で変わる事実（記録・ランキング・現役選手など）は基準年を明記する
2. 年を書かない場合は、時点依存しない一般知識に置き換える
3. 既存の雑学問題と重複していないかを確認する（同じ問いを別IDで増やさない）
